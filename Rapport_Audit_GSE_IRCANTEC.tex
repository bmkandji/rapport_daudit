...
\rowcolor{nxlightblue}
{\small Rapprochement des cibles} & {\small Écart cibles historiques vs induites par le modèle} & {\small V2F, BK, corrélations} & {\small Écart relatif $<$ [x]\,\%} \\
{\small Backtesting des moments simulés} & {\small Convergence trajectoires simulées vs paramètres d'entrée} & {\small Toutes classes ; quantiles 25 / 75\,\%} & {\small Écart $<$ [x] bp / pp} \\
\rowcolor{nxlightblue}
{\small Matrice simulée vs cible} & {\small Restitution des corrélations en sortie} & {\small Module corrélations} & {\small Écart absolu $<$ [x] pts} \\
{\small Stabilité inter-fenêtres} & {\small Variation des paramètres 15 vs 20 ans} & {\small Toutes classes} & {\small Variation commentée} \\
\rowcolor{nxlightblue}
{\small Condition de Feller / contrôles de schéma} & {\small Validité du schéma (positivité, convergence, monotonie)} & {\small CIR, Black-Karasinski} & {\small Condition vérifiée ou schéma justifié} \\
{\small Réplication des paramètres (run indépendant)} & {\small Écart paramètres recalculés vs production} & {\small Toutes composantes} & {\small Écart $<$ [tolérance]} \\
\end{longtable}
\vspace{-10pt}
...
\toprule
\tablehead{Driver} & \tablehead{Source} & \tablehead{Ticker / série} & \tablehead{Fréq.} & \tablehead{Première date} \\
\midrule\endfirsthead
\toprule
\tablehead{Driver} & \tablehead{Source} & \tablehead{Ticker / série} & \tablehead{Fréq.} & \tablehead{Première date} \\
\midrule\endhead
\bottomrule\endfoot
{\small Inflation (BEIR)} & {\small Bloomberg} & {\small GTFRFII2YR / GTFRFII10YR Corp} & {\small Mens.} & {\small 30/12/2005} \\
\end{longtable}
\vspace{-10pt}
...
\toprule
\tablehead{Étape} & \tablehead{Langage / outil} & \tablehead{Localisation} & \tablehead{Statut vérification} \\
\midrule\endfirsthead
\toprule
\tablehead{Étape} & \tablehead{Langage / outil} & \tablehead{Localisation} & \tablehead{Statut vérification} \\
\midrule\endhead
\bottomrule\endfoot
{\small Calibrage des paramètres} & {\small R} & {\small \ph{[chemin / script R]}} & {\small \ph{[à vérifier]}} \\
\rowcolor{nxlightblue}
{\small Simulation des trajectoires} & {\small MATLAB} & {\small \ph{[module GSE MATLAB]}} & {\small \ph{[à vérifier]}} \\
\end{longtable}
\vspace{-10pt}
...
\midrule\endhead
\bottomrule\endfoot
{\small $\kappa_2$ - vitesse retour moyenne (LT)} & {\small 26,18\,\%} & {\small 35,8\,\%} & {\small Facteur long terme} \\
\rowcolor{nxlightblue}
{\small $\sigma_2$ - volatilité (LT)} & {\small 1,06\,\%} & {\small 1,3\,\%} & {\small } \\
{\small $\mu_l$ - moyenne LT} & {\small 1,75\,\%} & {\small 1,75\,\%} & {\small Cible COR (inchangée)} \\
\rowcolor{nxlightblue}
{\small Valeur initiale (LT)} & {\small 1,806\,\%} & {\small 1,751\,\%} & {\small } \\
{\small $\kappa_1$ - vitesse retour moyenne (CT)} & {\small 78,01\,\%} & {\small 73,9\,\%} & {\small Facteur court terme} \\
\rowcolor{nxlightblue}
{\small $\sigma_1$ - volatilité (CT)} & {\small 1,567\,\%} & {\small 1,4\,\%} & {\small } \\
{\small Valeur initiale (CT)} & {\small 1,167\,\%} & {\small 1,725\,\%} & {\small } \\
\end{longtable}
\vspace{-10pt}
...
\toprule
\tablehead{Test / indicateur} & \tablehead{Objet} & \tablehead{Résultat} & \tablehead{Seuil / cible} & \tablehead{Conclusion} \\
\midrule\endfirsthead
\toprule
\tablehead{Test / indicateur} & \tablehead{Objet} & \tablehead{Résultat} & \tablehead{Seuil / cible} & \tablehead{Conclusion} \\
\midrule\endhead
\bottomrule\endfoot
{\small Shapiro-Wilk / Jarque-Bera} & {\small Normalité des résidus} & {\small [stat ; p-value]} & {\small p $>$ 5\,\%} & {\small [OK / KO]} \\
\rowcolor{nxlightblue}
{\small QQ-plot} & {\small Adéquation distributionnelle} & {\small [cf. figure]} & {\small Visuel} & {\small [OK / KO]} \\
{\small Ljung-Box / ACF (1--12)} & {\small Autocorrélation résiduelle} & {\small [stat ; p-value]} & {\small p $>$ 5\,\%} & {\small [OK / KO]} \\
\rowcolor{nxlightblue}
{\small Rapprochement des cibles} & {\small Cibles historiques vs modèle} & {\small [écarts]} & {\small $<$ [x]\,\%} & {\small [OK / KO]} \\
{\small Backtesting des moments simulés} & {\small Convergence moyenne / volatilité} & {\small [écarts]} & {\small $<$ [x] bp} & {\small [OK / KO]} \\
\rowcolor{nxlightblue}
{\small Réplication paramètres (run indépendant)} & {\small Écart vs production} & {\small [écart]} & {\small $<$ [tol.]} & {\small [OK / KO]} \\
\end{longtable}
\vspace{-10pt}
...
\toprule
\tablehead{Réf.} & \tablehead{Point d'attention} & \tablehead{Approche / modèle alternatif proposé} & \tablehead{Bénéfice attendu} \\
\midrule\endfirsthead
\toprule
\tablehead{Réf.} & \tablehead{Point d'attention} & \tablehead{Approche / modèle alternatif proposé} & \tablehead{Bénéfice attendu} \\
\midrule\endhead
\bottomrule\endfoot
{\small IN-A1} & {\small Cible $\mu_l$ fixée à dire d'expert (COR) hors calibrage statistique} & {\small Documenter / encadrer la cible par un intervalle ; analyse de sensibilité de l'allocation à $\mu_l$} & {\small Traçabilité ; maîtrise du risque d'indexation} \\
\rowcolor{nxlightblue}
{\small IN-A2} & {\small V2F gaussien : pas de garantie de cohérence des queues d'inflation (épisode 2022)} & {\small Modèle à volatilité stochastique ou à sauts ; cible additionnelle sur les quantiles extrêmes} & {\small Meilleure capture des scénarios d'inflation élevée} \\
{\small IN-A3} & {\small Corrélation $\rho$ entre browniens fixée à 0 dans la remédiation} & {\small Estimer $\rho$ ou le calibrer sur une cible de corrélation CT/LT} & {\small Structure de dépendance interne plus réaliste} \\
\end{longtable}
\vspace{-10pt}
...
\midrule\endhead
\bottomrule\endfoot
{\small Taux nominaux} & {\small Bloomberg} & {\small GFRN2 / GFRN10 Index} & {\small Mens.} & {\small 30/12/2005} \\
\rowcolor{nxlightblue}
{\small Inflation (BEIR)} & {\small Bloomberg} & {\small GTFRFII2YR / GTFRFII10YR Corp} & {\small Mens.} & {\small 30/12/2005} \\
{\small Taux réels} & {\small Dérivé} & {\small Relation de Fisher $(1+R^n)/(1+i)-1$} & {\small Mens.} & {\small -} \\
\end{longtable}
\vspace{-10pt}
...
\midrule\endhead
\bottomrule\endfoot
{\small Calibrage des paramètres} & {\small R} & {\small \ph{[script R]}} & {\small \ph{[à vérifier]}} \\
\rowcolor{nxlightblue}
{\small Reconstruction Fisher} & {\small \ph{[R / MATLAB ?]}} & {\small \ph{[à préciser]}} & {\small \ph{[à vérifier]}} \\
{\small Simulation des trajectoires} & {\small MATLAB} & {\small \ph{[module GSE]}} & {\small \ph{[à vérifier]}} \\
\end{longtable}
\vspace{-10pt}
\tabcaption{Localisation des implémentations et statut de vérification}
\subsubsection{Paramètres en production (allocation 2026 vs 2022)}
...
\midrule\endhead
\bottomrule\endfoot
{\small $\kappa_2$ - vitesse retour moyenne (LT)} & {\small 11,89\,\%} & {\small 10,8\,\%} & {\small Facteur long terme} \\
\rowcolor{nxlightblue}
{\small $\sigma_2$ - volatilité (LT)} & {\small 1,086\,\%} & {\small 0,9\,\%} & {\small } \\
{\small $\mu$ - moyenne LT} & {\small 0,41\,\%} & {\small $-$0,2\,\%} & {\small Passe en territoire positif} \\
\rowcolor{nxlightblue}
{\small Valeur initiale (LT)} & {\small 1,82\,\%} & {\small $-$1,515\,\%} & {\small Forte révision (remontée des taux)} \\
{\small $\kappa_1$ - vitesse retour moyenne (CT)} & {\small 61,75\,\%} & {\small 60,0\,\%} & {\small Facteur court terme} \\
\rowcolor{nxlightblue}
{\small $\sigma_1$ - volatilité (CT)} & {\small 1,545\,\%} & {\small 1,3\,\%} & {\small } \\
{\small Valeur initiale (CT)} & {\small 1,146\,\%} & {\small $-$2,379\,\%} & {\small Forte révision} \\
\end{longtable}
\vspace{-10pt}
\tabcaption{Paramètres en production - valeurs extraites du classeur \texttt{Template\_Allocationv2.xlsm}}
\begin{auditbox}
...
\midrule\endhead
\bottomrule\endfoot
{\small TR-A1} & {\small Abandon de l'\og effet miroir \fg{} : révision très forte de la valeur initiale et de $\mu$ (cf. 2022)} & {\small Documenter la transition ; cohérence du point de départ avec la courbe initiale} & {\small Robustesse du niveau initial des taux} \\
\rowcolor{nxlightblue}
{\small TR-A2} & {\small Reconstruction Fisher : conventions et primes d'inflation incorporées dans le BEIR} & {\small Confronter aux OATi directes ; expliciter la prime de risque d'inflation} & {\small Cohérence réel / nominal / inflation} \\
{\small TR-A3} & {\small V2F autorise des taux réels arbitrairement négatifs sur 30 ans} & {\small Acceptable, mais à encadrer (bornes $\pm$90\,\% présentes dans le classeur)} & {\small Maîtrise des scénarios extrêmes} \\
\end{longtable}
\vspace{-10pt}
...
\midrule\endhead
\bottomrule\endfoot
{\small Corrél. taux réel $\times$ inflation $\rho_{r,i}$} & {\small Module corrél.} & {\small Hist. $\approx -50\,\%$ ; sensi. $-70\,\%$} & {\small Mens.} & {\small -} \\
\rowcolor{nxlightblue}
{\small Corrél. taux court $\times$ long $\rho_{r,l}$} & {\small Production} & {\small $-22,65\,\%$ ; sensi. $+70\,\%$} & {\small -} & {\small -} \\
{\small Primes de risque $\gamma_1, \gamma_2$} & {\small Cibles de niveau} & {\small \ph{[à documenter]}} & {\small -} & {\small -} \\
\end{longtable}
\vspace{-10pt}
...
\midrule\endhead
\bottomrule\endfoot
{\small Calcul prix ZC + corrections} & {\small \ph{[à préciser : MATLAB ?]}} & {\small \ph{[à préciser]}} & {\small \ph{[à vérifier]}} \\
\rowcolor{nxlightblue}
{\small Test de sensibilité ($\rho$)} & {\small \ph{[à préciser]}} & {\small \ph{[à préciser]}} & {\small \ph{[à vérifier]}} \\
\end{longtable}
\vspace{-10pt}
\tabcaption{Localisation des implémentations et statut de vérification}
\subsubsection{Paramètres en production (allocation 2026 vs 2022)}
...
\toprule
\tablehead{Configuration de pricing} & \tablehead{Effet rdt (bp, 10 ans)} & \tablehead{Effet vol. (bp)} & \tablehead{Commentaire} \\
\midrule\endfirsthead
\toprule
\tablehead{Configuration de pricing} & \tablehead{Effet rdt (bp, 10 ans)} & \tablehead{Effet vol. (bp)} & \tablehead{Commentaire} \\
\midrule\endhead
\bottomrule\endfoot
{\small (1) Covariance taux réel $\times$ inflation ($\rho_{r,i} \approx -50\%$)} & {\small $\approx$ +10} & {\small $\approx$ 0} & {\small Vs hypothèse d'indépendance} \\
\rowcolor{nxlightblue}
{\small (2) Covariance taux court $\times$ long ($\rho_{r,l}$)} & {\small $\approx$ +7} & {\small $\approx$ 0} & {\small Nouveau facteur de covariance} \\
{\small Cumul (1)+(2)} & {\small $\approx$ +17} & {\small $\approx$ 0} & {\small Rendement ZC nominal 10 ans} \\
\rowcolor{nxlightblue}
{\small Sensibilité $\rho_{r,i}=-70\%$} & {\small $\approx$ +15} & {\small \ph{[à compl.]}} & {\small Test de sensibilité} \\
{\small Sensibilité $\rho_{r,i}=-70\%$ \& $\rho_{r,l}=+70\%$} & {\small $\approx$ +23} & {\small \ph{[à compl.]}} & {\small Test combiné} \\
\end{longtable}
\vspace{-10pt}
\tabcaption{Paramètres en production - valeurs extraites du classeur \texttt{Template\_Allocationv2.xlsm}}
\begin{auditbox}
...
\toprule
\tablehead{Hypothèse} & \tablehead{Couples concernés} & \tablehead{Valeur} & \tablehead{Justification documentée} \\
\midrule\endfirsthead
\toprule
\tablehead{Hypothèse} & \tablehead{Couples concernés} & \tablehead{Valeur} & \tablehead{Justification documentée} \\
\midrule\endhead
\bottomrule\endfoot
{\small Décorrélation interne d'un V2F} & {\small Inflation CT $\times$ Inflation LT ; Taux réel CT $\times$ Taux réel LT} & {\small 0\,\%} & {\small Convention de modélisation (browniens $W^1 \perp W^2$ du V2F) ; \textbf{non justifiée empiriquement}} \\
\rowcolor{nxlightblue}
{\small Adossement du facteur court au facteur long} & {\small Corrélations du facteur \emph{court} (infl. CT, taux réel CT) avec toutes les autres classes} & {\small $=$ corrél. du facteur \emph{long} correspondant} & {\small Le court hérite des corrélations du long (ex. Action $\times$ Infl.\,CT $=$ Action $\times$ Infl.\,LT $= 21,5\%$)} \\
{\small Décorrélation croisée inter-V2F} & {\small Infl.\,LT $\times$ Taux réel CT ; Infl.\,CT $\times$ Taux réel LT} & {\small 0\,\%} & {\small Seul le couple long/long (Infl.\,LT $\times$ Taux réel LT $= -22,65\%$) porte la dépendance inflation / taux réel} \\
\rowcolor{nxlightblue}
{\small Corrélation identique aux 2 facteurs d'un V2F} & {\small Formules de passage $\rho^{hist}\to\rho^{GSE}$ (V2F $\times$ autre driver)} & {\small $\rho_{\varepsilon^1} = \rho_{\varepsilon^2}$} & {\small Hypothèse simplificatrice des formules de passage (cf. annexes Milliman 2022)} \\
{\small Corrélation taux court / taux long (pricing ZC)} & {\small $\rho_{r,l}$ dans $\operatorname{Var}[\int r]$} & {\small $-22,65\,\%$} & {\small Auparavant supposée nulle ; introduite en 2026 pour le pricing} \\
\rowcolor{nxlightblue}
{\small Action stressée $\times$ action normale} & {\small Action Euro/Monde normale $\times$ stressée} & {\small 0\,\% \ph{[à vérifier]}} & {\small Non utilisée dans la simulation (régimes séparés) - convention à confirmer} \\
\end{longtable}
\vspace{-10pt}
...
\midrule\endhead
\bottomrule\endfoot
{\small Vérif. formules $A, B_1, B_2$} & {\small Exactitude du pricing fermé V2F} & {\small [OK / écarts]} & {\small Identité analytique} & {\small [OK / KO]} \\
\rowcolor{nxlightblue}
{\small Vérif. terme de covariance (1)} & {\small Exactitude de la correction Fisher $\rho_{r,i}$} & {\small [OK / écarts]} & {\small Identité analytique} & {\small [OK / KO]} \\
{\small Vérif. terme de covariance (2)} & {\small Exactitude du terme $\rho_{r,l}\sqrt{\operatorname{Var}[I]\operatorname{Var}[J]}$} & {\small [OK / écarts]} & {\small Identité analytique} & {\small [OK / KO]} \\
\rowcolor{nxlightblue}
{\small Contre-implémentation MC} & {\small Prix simulé vs formule fermée} & {\small [écart bp]} & {\small $<$ [x] bp} & {\small [OK / KO]} \\
{\small Sensibilité $\rho$} & {\small Impact sur rendement ZC} & {\small [cf. tableau]} & {\small Documenté} & {\small [OK / KO]} \\
\end{longtable}
\vspace{-10pt}
...
\midrule\endhead
\bottomrule\endfoot
{\small ZC-A1} & {\small Corrélation $\rho_{r,i}$ taux réel / inflation : impact matériel ($\sim$+10 à +15 bp)} & {\small Calibrer $\rho_{r,i}$ sur cible historique plutôt que valeur d'expert ; suivi dans le temps} & {\small Pricing ZC robuste et traçable} \\
\rowcolor{nxlightblue}
{\small ZC-A2} & {\small Primes de risque $\gamma_1, \gamma_2$ supposées constantes} & {\small Primes dépendantes de la maturité ou du temps ; calibrage multi-points de courbe} & {\small Meilleur ajustement de la courbe initiale} \\
{\small ZC-A3} & {\small Approximation de Fisher (additivité) pour des taux non négligeables} & {\small Relation de Fisher exacte ; quantifier l'erreur résiduelle} & {\small Précision du pricing} \\
\rowcolor{nxlightblue}
{\small ZC-A4} & {\small Absence de garantie de non-arbitrage le long des trajectoires} & {\small Contrôle de cohérence risque-neutre ; reprojection des prix} & {\small Cohérence financière} \\
\end{longtable}
\vspace{-10pt}
...
\midrule\endhead
\bottomrule\endfoot
{\small Spread IG (5--7 ans)} & {\small Bloomberg} & {\small H06270EU Index} & {\small Mens.} & {\small 29/12/2000} \\
\rowcolor{nxlightblue}
{\small CDS (prime de contrepartie)} & {\small \ph{[source]}} & {\small \ph{[ticker]}} & {\small Mens.} & {\small \ph{[date]}} \\
\end{longtable}
\vspace{-10pt}
...
{\small Calibrage des paramètres} & {\small R} & {\small \ph{[script R]}} & {\small \ph{[à vérifier]}} \\
\rowcolor{nxlightblue}
{\small Simulation (schéma Alfonsi)} & {\small MATLAB} & {\small \ph{[module GSE]}} & {\small \ph{[à vérifier]}} \\
\end{longtable}
\vspace{-10pt}
\tabcaption{Localisation des implémentations et statut de vérification}
\subsubsection{Paramètres en production (allocation 2026 vs 2022)}
...
\midrule\endhead
\bottomrule\endfoot
{\small $\kappa$ - vitesse retour moyenne} & {\small 52,59\,\%} & {\small 38,0\,\%} & {\small } \\
\rowcolor{nxlightblue}
{\small $\sigma$ - volatilité} & {\small 5,167\,\%} & {\small 4,8\,\%} & {\small } \\
{\small $\theta$ - moyenne LT} & {\small 1,42\,\%} & {\small 1,3\,\%} & {\small } \\
\rowcolor{nxlightblue}
{\small Valeur initiale} & {\small 0,786\,\%} & {\small 0,926\,\%} & {\small Niveau de spread au 31/12} \\
{\small Taux de défaut annuel} & {\small 0,224\,\%} & {\small 0,2\,\%} & {\small À dire d'expert (hors périmètre)} \\
\rowcolor{nxlightblue}
{\small Taux de recouvrement} & {\small 50,0\,\%} & {\small 50,0\,\%} & {\small À dire d'expert (hors périmètre)} \\
\end{longtable}
\vspace{-10pt}
\tabcaption{Paramètres en production - valeurs extraites du classeur \texttt{Template\_Allocationv2.xlsm}}
\begin{auditbox}
...
\rowcolor{nxlightblue}
{\small Réplication paramètres (run indépendant)} & {\small Écart vs production} & {\small [écart]} & {\small $<$ [tol.]} & {\small [OK / KO]} \\
{\small Condition $\sigma^2 < 4\kappa\theta$ (Alfonsi)} & {\small Validité du schéma $E(0)$} & {\small [vérifiée / non]} & {\small $\sigma^2 < 4\kappa\theta$} & {\small [OK / KO]} \\
\rowcolor{nxlightblue}
{\small Convergence du schéma} & {\small Convergence forte / faible} & {\small [ordres]} & {\small Documenté} & {\small [OK / KO]} \\
\end{longtable}
\vspace{-10pt}
...
\midrule\endhead
\bottomrule\endfoot
{\small CR-A1} & {\small Forte révision de $\kappa$ (52,6\,\% vs 38,0\,\%) : instabilité du calibrage} & {\small Vraisemblance profilée ; intervalle de confiance ; correction du biais de réversion en échantillon fini} & {\small Robustesse de la dispersion LT des spreads} \\
\rowcolor{nxlightblue}
{\small CR-A2} & {\small Un seul facteur de spread (IG) ; pas de risque de migration} & {\small Multi-rating (HY) avec corrélations ; modèle JLT pour les migrations} & {\small Granularité du risque de crédit} \\
{\small CR-A3} & {\small Intensité de défaut déterministe} & {\small CIR sur l'intensité de défaut, cohérent avec les spreads en monde réel} & {\small Cohérence défaut / spread} \\
\end{longtable}
\vspace{-10pt}
...
\midrule\endhead
\bottomrule\endfoot
{\small Spread dette privée} & {\small Bloomberg} & {\small LP01TREU Index} & {\small Mens.} & {\small 29/12/2000} \\
\end{longtable}
\vspace{-10pt}
...
\midrule\endhead
\bottomrule\endfoot
{\small Calibrage des paramètres} & {\small Excel} & {\small \texttt{Spread liqui dette prive}} & {\small \ph{[à vérifier]}} \\
\rowcolor{nxlightblue}
{\small Simulation des trajectoires} & {\small MATLAB} & {\small \ph{[module GSE]}} & {\small \ph{[à vérifier]}} \\
\end{longtable}
\vspace{-10pt}
...
\midrule\endhead
\bottomrule\endfoot
{\small $k$ - vitesse retour moyenne} & {\small 43,64\,\%} & {\small 34,7\,\%} & {\small Sur log-spread} \\
\rowcolor{nxlightblue}
{\small $\sigma$ - volatilité (log-spread)} & {\small 49,67\,\%} & {\small 24,1\,\%} & {\small Forte hausse} \\
{\small $\theta$ - moyenne (log-spread)} & {\small $-$3,109} & {\small $-$3,072} & {\small } \\
\rowcolor{nxlightblue}
{\small Valeur initiale} & {\small 2,814\,\%} & {\small 2,91\,\%} & {\small Niveau de spread} \\
{\small Prime de liquidité (surplus)} & {\small 4,64\,\%} & {\small -} & {\small Dire d'expert + ajustement $\theta$} \\
\end{longtable}
\vspace{-10pt}
\tabcaption{Paramètres en production - valeurs extraites du classeur \texttt{Template\_Allocationv2.xlsm}}
\begin{auditbox}
...
\midrule\endhead
\bottomrule\endfoot
{\small DP-A1} & {\small Très forte hausse et instabilité de $\sigma$ (49,7\,\% vs 24,1\,\%)} & {\small Vérifier la fenêtre et l'indice ; intervalle de confiance ; cibles distributionnelles en alternative} & {\small Maîtrise de la volatilité diffusée} \\
\rowcolor{nxlightblue}
{\small DP-A2} & {\small Caractère log-normal potentiellement explosif sur 30 ans} & {\small Contrôle des moments d'ordre supérieur ; mean-reversion renforcée} & {\small Stabilité des projections longues} \\
{\small DP-A3} & {\small Proxy coté (leveraged loans) pour de la dette privée non cotée} & {\small Indice plus représentatif ou ajustement de prime ; unsmoothing si lissage} & {\small Adéquation au portefeuille réel} \\
\end{longtable}
\vspace{-10pt}
...
\midrule\endhead
\bottomrule\endfoot
{\small CDS} & {\small \ph{[source]}} & {\small \ph{[ticker]}} & {\small Mens.} & {\small \ph{[date]}} \\
\rowcolor{nxlightblue}
{\small Spread oblig. / dette privée} & {\small Bloomberg} & {\small H06270EU / LP01TREU} & {\small Mens.} & {\small 29/12/2000} \\
\end{longtable}
\vspace{-10pt}
...
\midrule\endhead
\bottomrule\endfoot
{\small Calcul prime de liquidité} & {\small Excel} & {\small \texttt{Spread liqui dette prive}} & {\small \ph{[à vérifier]}} \\
\end{longtable}
\vspace{-10pt}
\tabcaption{Localisation des implémentations et statut de vérification}
\subsubsection{Paramètres en production (allocation 2026 vs 2022)}
...
\toprule
\tablehead{Grandeur} & \tablehead{CDS} & \tablehead{Spread 20 ans} & \tablehead{Commentaire} \\
\midrule\endfirsthead
\toprule
\tablehead{Grandeur} & \tablehead{CDS} & \tablehead{Spread 20 ans} & \tablehead{Commentaire} \\
\midrule\endhead
\bottomrule\endfoot
{\small Vitesse retour moyenne $k$} & {\small 1,234} & {\small 0,436} & {\small Sur log-spread} \\
\rowcolor{nxlightblue}
{\small Volatilité $\sigma$} & {\small 65,97\,\%} & {\small 49,67\,\%} & {\small } \\
{\small Moyenne $\theta$ (log)} & {\small $-$3,461} & {\small $-$3,156} & {\small } \\
\rowcolor{nxlightblue}
{\small Moyenne LT du spread} & {\small 2,875\,\%} & {\small 3,699\,\%} & {\small } \\
{\small Prime liquidité High Yield} & {\small -} & {\small 0,824\,\%} & {\small Calculée} \\
\rowcolor{nxlightblue}
{\small Prime liquidité dette privée} & {\small -} & {\small 1,0\,\%} & {\small Dire d'expert} \\
{\small Surplus de moyenne} & {\small -} & {\small 4,639\,\%} & {\small Intégré à $\theta$} \\
\end{longtable}
\vspace{-10pt}
\tabcaption{Paramètres en production - valeurs extraites du classeur \texttt{Template\_Allocationv2.xlsm}}
\begin{auditbox}
...
\midrule\endhead
\bottomrule\endfoot
{\small Cohérence du différentiel} & {\small Prime $\geq$ 0 ; ordre de grandeur de place} & {\small [valeur]} & {\small Plausible} & {\small [OK / KO]} \\
\rowcolor{nxlightblue}
{\small Stabilité de la prime} & {\small Sensibilité à la fenêtre (15 vs 20 ans)} & {\small [écart]} & {\small Commenté} & {\small [OK / KO]} \\
{\small Réplication (run indépendant)} & {\small Écart vs production} & {\small [écart]} & {\small $<$ [tol.]} & {\small [OK / KO]} \\
\end{longtable}
\vspace{-10pt}
...
\midrule\endhead
\bottomrule\endfoot
{\small LQ-A1} & {\small Prime de liquidité dette privée fixée à dire d'expert (1,0\,\%)} & {\small Documenter le rationnel ; encadrer par un intervalle ; benchmark de place} & {\small Traçabilité et auditabilité} \\
\rowcolor{nxlightblue}
{\small LQ-A2} & {\small Prime constante dans le temps} & {\small Prime de liquidité variable selon le cycle / régime de marché} & {\small Réalisme en conditions stressées} \\
{\small LQ-A3} & {\small Séparation contrepartie / liquidité dépendante de la qualité des CDS} & {\small Vérifier l'appariement CDS / obligataire (rating, maturité, séniorité)} & {\small Robustesse de la décomposition} \\
\end{longtable}
\vspace{-10pt}
...
\midrule\endhead
\bottomrule\endfoot
{\small Action Euro} & {\small Bloomberg} & {\small MSDEE15N Index} & {\small Mens.} & {\small 29/12/2000} \\
\rowcolor{nxlightblue}
{\small Action Monde ex-Europe} & {\small Bloomberg} & {\small MSDEWEUN Index} & {\small Mens.} & {\small 29/12/2000} \\
{\small Action Émergent} & {\small Bloomberg} & {\small MSDEEEMN Index} & {\small Mens.} & {\small 29/12/2000} \\
\end{longtable}
\vspace{-10pt}
...
\toprule
\tablehead{Paramètre (20 ans)} & \tablehead{Euro} & \tablehead{Monde} & \tablehead{Émergent} \\
\midrule\endfirsthead
\toprule
\tablehead{Paramètre (20 ans)} & \tablehead{Euro} & \tablehead{Monde} & \tablehead{Émergent} \\
\midrule\endhead
\bottomrule\endfoot
{\small $p_{1\to2}$ (sortie régime 1)} & {\small 14,83\,\%} & {\small 14,73\,\%} & {\small 15,31\,\%} \\
\rowcolor{nxlightblue}
{\small $p_{2\to1}$ (sortie régime 2)} & {\small 5,27\,\%} & {\small 5,64\,\%} & {\small 4,80\,\%} \\
{\small Moyenne régime 1 (stressé)} & {\small $-$13,17\,\%} & {\small $-$12,18\,\%} & {\small $-$13,61\,\%} \\
\rowcolor{nxlightblue}
{\small Volatilité régime 1 (stressé)} & {\small 21,76\,\%} & {\small 19,83\,\%} & {\small 27,05\,\%} \\
{\small Moyenne régime 2 (normal)} & {\small 13,61\,\%} & {\small 18,01\,\%} & {\small 13,13\,\%} \\
\rowcolor{nxlightblue}
{\small Volatilité régime 2 (normal)} & {\small 9,52\,\%} & {\small 9,08\,\%} & {\small 11,06\,\%} \\
{\small Proba. stationnaire de stress} & {\small 26,24\,\%} & {\small 27,68\,\%} & {\small 23,85\,\%} \\
\end{longtable}
\vspace{-10pt}
\tabcaption{Paramètres en production - valeurs extraites du classeur \texttt{Template\_Allocationv2.xlsm}}
\begin{auditbox}
...
\rowcolor{nxlightblue}
{\small Réplication paramètres (run indépendant)} & {\small Écart vs production} & {\small [écart]} & {\small $<$ [tol.]} & {\small [OK / KO]} \\
{\small Convergence des $p_{ij}$ simulés} & {\small Restitution des probabilités de transition} & {\small [écarts]} & {\small $<$ [x] pts} & {\small [OK / KO]} \\
\rowcolor{nxlightblue}
{\small Prime de risque / vol. LT} & {\small Cohérence vs empirique et projections de place} & {\small [valeurs]} & {\small Ordres de grandeur} & {\small [OK / KO]} \\
\end{longtable}
\vspace{-10pt}
...
\midrule\endhead
\bottomrule\endfoot
{\small AC-A1} & {\small RSLN-2 à 2 régimes : queues éventuellement sous-estimées} & {\small RSLN-3 (3 régimes) ou modèle à sauts ; cible additionnelle sur les quantiles} & {\small Meilleure capture des crises} \\
\rowcolor{nxlightblue}
{\small AC-A2} & {\small Indices NR : effet de change non explicitement modélisé (Monde, Émergent)} & {\small Distinguer rendement local et effet de change ; couverture partielle} & {\small Réalisme de l'exposition} \\
{\small AC-A3} & {\small Action Monde corrélée par abattement à l'Euro (cf. corrélations)} & {\small Calibrage direct des corrélations de l'action Monde} & {\small Contrôle de la structure de dépendance} \\
\end{longtable}
\vspace{-10pt}
...
\midrule\endhead
\bottomrule\endfoot
{\small Action non cotée} & {\small Bloomberg} & {\small LPX50TR Index} & {\small Mens.} & {\small 29/12/2000} \\
\end{longtable}
\vspace{-10pt}
...
\midrule\endhead
\bottomrule\endfoot
{\small Calibrage (unsmoothing + BS)} & {\small Excel} & {\small \ph{[onglet / classeur]}} & {\small \ph{[à vérifier]}} \\
\rowcolor{nxlightblue}
{\small Simulation des trajectoires} & {\small MATLAB} & {\small \ph{[module GSE]}} & {\small \ph{[à vérifier]}} \\
\end{longtable}
\vspace{-10pt}
...
\midrule\endhead
\bottomrule\endfoot
{\small $\mu$ - rendement moyen (PE)} & {\small 7,18\,\%} & {\small Régime faible volatilité : 5,971\,\%} \\
\rowcolor{nxlightblue}
{\small $\sigma$ - volatilité (post-unsmoothing)} & {\small 29,70\,\%} & {\small Régime faible volatilité : 15,0\,\%} \\
{\small Rendement \og Ncote composite \fg{} (sortie)} & {\small 6,71\,\%} & {\small Onglet OutPut (PE + dette privée)} \\
\rowcolor{nxlightblue}
{\small Volatilité \og Ncote composite \fg{} (sortie)} & {\small 15,89\,\%} & {\small Onglet OutPut} \\
\end{longtable}
\vspace{-10pt}
\tabcaption{Paramètres en production - valeurs extraites du classeur \texttt{Template\_Allocationv2.xlsm}}
\begin{auditbox}
...
\rowcolor{nxlightblue}
{\small Réplication paramètres (run indépendant)} & {\small Écart vs production} & {\small [écart]} & {\small $<$ [tol.]} & {\small [OK / KO]} \\
{\small ACF post-unsmoothing} & {\small Annulation de l'autocorrélation d'ordre 1 ; ordres $\geq$ 2} & {\small [ACF(1)]} & {\small ACF(1) $\approx$ 0} & {\small [OK / KO]} \\
\end{longtable}
\vspace{-10pt}
...
\midrule\endhead
\bottomrule\endfoot
{\small PE-A1} & {\small Proxy coté (bêta de marché élevé) pour du non coté} & {\small Indice plus représentatif (millésimes, stratégies) ou modèle à bêta ajusté} & {\small Adéquation au portefeuille réel} \\
\rowcolor{nxlightblue}
{\small PE-A2} & {\small Unsmoothing limité à l'ordre 1} & {\small Unsmoothing aux ordres supérieurs (cf. Lizieri et al. 2012)} & {\small Volatilité non biaisée} \\
{\small PE-A3} & {\small Agrégation \og composite \fg{} PE + dette privée} & {\small Documenter la pondération et la cohérence de l'agrégat} & {\small Traçabilité de la classe composite} \\
\end{longtable}
\vspace{-10pt}
...
\midrule\endhead
\bottomrule\endfoot
{\small Immobilier} & {\small EDHEC IEIF (ieif.fr)} & {\small Indice immobilier entreprise France} & {\small Mensuel} & {\small 30/06/2008} \\
\end{longtable}
\vspace{-10pt}
...
\midrule\endhead
\bottomrule\endfoot
{\small $\mu$ - rendement moyen} & {\small 5,633\,\%} & {\small 6,288\,\%} & {\small } \\
\rowcolor{nxlightblue}
{\small $\sigma$ - volatilité (post-unsmoothing)} & {\small 6,128\,\%} & {\small 7,9\,\%} & {\small } \\
{\small Rendement (sortie OutPut)} & {\small 5,613\,\%} & {\small -} & {\small Restitution simulation} \\
\rowcolor{nxlightblue}
{\small Volatilité (sortie OutPut)} & {\small 6,158\,\%} & {\small -} & {\small Restitution simulation} \\
\end{longtable}
\vspace{-10pt}
\tabcaption{Paramètres en production - valeurs extraites du classeur \texttt{Template\_Allocationv2.xlsm}}
\begin{auditbox}
...
\rowcolor{nxlightblue}
{\small Réplication paramètres (run indépendant)} & {\small Écart vs production} & {\small [écart]} & {\small $<$ [tol.]} & {\small [OK / KO]} \\
{\small ACF post-unsmoothing} & {\small Annulation de l'autocorrélation d'ordre 1} & {\small [valeurs]} & {\small ACF(1) $\approx$ 0} & {\small [OK / KO]} \\
\end{longtable}
\vspace{-10pt}
...
\midrule\endhead
\bottomrule\endfoot
{\small IM-A1} & {\small BS gaussien : cycles immobiliers longs et retour à la moyenne non modélisés} & {\small Modèle à retour à la moyenne sur le rendement ou à régimes} & {\small Réalisme des cycles immobiliers} \\
\rowcolor{nxlightblue}
{\small IM-A2} & {\small Profondeur 2008+ : fenêtre \og 20 ans \fg{} tronquée à $\sim$17 ans} & {\small Clarifier la fenêtre effective ; cibles d'expert de long terme} & {\small Cohérence de la fenêtre} \\
{\small IM-A3} & {\small Volatilité 6\,\% post-unsmoothing possiblement sous-estimée} & {\small Unsmoothing aux ordres supérieurs ; comparaison à l'immobilier coté} & {\small Couple rendement / risque non biaisé} \\
\end{longtable}
\vspace{-10pt}
...
\midrule\endhead
\bottomrule\endfoot
{\small Infrastructure} & {\small Bloomberg} & {\small NMX30T Index} & {\small Mens.} & {\small 29/12/2000} \\
\end{longtable}
\vspace{-10pt}
...
\midrule\endhead
\bottomrule\endfoot
{\small $\mu$ - rendement moyen} & {\small 6,516\,\%} & {\small 7,626\,\%} & {\small } \\
\rowcolor{nxlightblue}
{\small $\sigma$ - volatilité (post-unsmoothing)} & {\small 13,498\,\%} & {\small 18,459\,\%} & {\small } \\
{\small Rendement (sortie OutPut)} & {\small 6,55\,\%} & {\small -} & {\small Restitution simulation} \\
\rowcolor{nxlightblue}
{\small Volatilité (sortie OutPut)} & {\small 13,578\,\%} & {\small -} & {\small Restitution simulation} \\
\end{longtable}
\vspace{-10pt}
\tabcaption{Paramètres en production - valeurs extraites du classeur \texttt{Template\_Allocationv2.xlsm}}
\begin{auditbox}
...
\midrule\endhead
\bottomrule\endfoot
{\small IF-A1} & {\small Indice coté NMX30 : profil proche des actions (corrélations 72--92\,\%)} & {\small Indice / modèle plus spécifique à l'infrastructure non cotée} & {\small Contribution réelle à la diversification} \\
\rowcolor{nxlightblue}
{\small IF-A2} & {\small Hypothèse BS gaussien} & {\small Modèle à retour à la moyenne des rendements} & {\small Réalisme des cash-flows régulés} \\
\end{longtable}
\vspace{-10pt}
...
\midrule\endhead
\bottomrule\endfoot
{\small Tous facteurs} & {\small Cf. \S\ref{sec:carto}} & {\small Séries du GSE (15 drivers)} & {\small Mens.} & {\small 31/12/2025} \\
\end{longtable}
\vspace{-10pt}
...
\midrule\endhead
\bottomrule\endfoot
{\small Estimation + passage browniens} & {\small Python} & {\small \ph{[script Python]}} & {\small \ph{[à vérifier]}} \\
\rowcolor{nxlightblue}
{\small Régularisation Higham} & {\small Python} & {\small \ph{[script Python]}} & {\small \ph{[à vérifier]}} \\
{\small Cholesky + diffusion corrélée} & {\small MATLAB} & {\small \ph{[module GSE]}} & {\small \ph{[à vérifier]}} \\
\end{longtable}
\vspace{-10pt}
\tabcaption{Localisation des implémentations et statut de vérification}
\subsubsection{Paramètres en production (allocation 2026 vs 2022)}
...
\toprule
\tablehead{Couple de facteurs (extrait)} & \tablehead{Corrél. browniens 2026} & \tablehead{Commentaire} \\
\midrule\endfirsthead
\toprule
\tablehead{Couple de facteurs (extrait)} & \tablehead{Corrél. browniens 2026} & \tablehead{Commentaire} \\
\midrule\endhead
\bottomrule\endfoot
{\small Action Euro $\times$ Action Monde} & {\small 82,85\,\%} & {\small Cohérent (forte corrélation actions)} \\
\rowcolor{nxlightblue}
{\small Action Euro $\times$ Spread crédit} & {\small $-$71,47\,\%} & {\small Corrélation négative attendue} \\
{\small Action Euro $\times$ Dette privée} & {\small $-$72,52\,\%} & {\small } \\
\rowcolor{nxlightblue}
{\small Inflation LT $\times$ Taux réel LT} & {\small $-$22,65\,\%} & {\small Cohérent avec la littérature} \\
{\small Spread crédit $\times$ Dette privée} & {\small 90,83\,\%} & {\small Très élevée (proximité des risques)} \\
\rowcolor{nxlightblue}
{\small Action Monde $\times$ Action Monde stressée} & {\small 0,0\,\%} & {\small \textbf{À vérifier} (convention ou anomalie)} \\
{\small Private Equity $\times$ Infrastructure} & {\small 72,66\,\%} & {\small } \\
\end{longtable}
\vspace{-10pt}
\tabcaption{Paramètres en production - valeurs extraites du classeur \texttt{Template\_Allocationv2.xlsm}}
\begin{auditbox}
...
\midrule\endhead
\bottomrule\endfoot
{\small Matrice simulée vs cible} & {\small Restitution des corrélations en sortie} & {\small [écarts]} & {\small $<$ [x] pts} & {\small [OK / KO]} \\
\rowcolor{nxlightblue}
{\small Impact Higham} & {\small Distance matrice cible / SDP} & {\small [Frobenius]} & {\small Documenté} & {\small [OK / KO]} \\
{\small Cohérence économique} & {\small Signes et magnitudes vs littérature} & {\small [revue]} & {\small Cohérent} & {\small [OK / KO]} \\
\rowcolor{nxlightblue}
{\small Vérif. formules de passage} & {\small Contre-dérivation des conversions} & {\small [OK / écarts]} & {\small Identité analytique} & {\small [OK / KO]} \\
{\small Valeurs atypiques} & {\small Corrélations à 0\,\% normal / stressé} & {\small [contrôle]} & {\small Convention justifiée} & {\small [OK / KO]} \\
\end{longtable}
\vspace{-10pt}
...
\midrule\endhead
\bottomrule\endfoot
{\small CO-A1} & {\small Copule gaussienne : pas de dépendance de queue} & {\small Copule à dépendance de queue (Student, mélanges) pour le stress} & {\small Réalisme des co-mouvements en crise} \\
\rowcolor{nxlightblue}
{\small CO-A2} & {\small Corrélations \og normal \fg{} vs \og stressé \fg{} à 0\,\% (Action Monde)} & {\small Vérifier convention / séparer les matrices par régime} & {\small Exactitude de la structure de dépendance} \\
{\small CO-A3} & {\small Décorrélations structurelles fixées à 0 (CT $\times$ LT d'un V2F ; croisées inter-V2F)} & {\small Estimer ces corrélations sur historique ; mesurer l'impact d'une valeur non nulle} & {\small Structure de dépendance non sur-contrainte} \\
\rowcolor{nxlightblue}
{\small CO-A4} & {\small Adossement du facteur court aux corrélations du facteur long} & {\small Calibrer directement les corrélations du facteur court} & {\small Précision de la dépendance court terme} \\
{\small CO-A5} & {\small Hypothèse de corrélation identique aux deux facteurs d'un V2F (formules de passage)} & {\small Lever l'hypothèse ; calibrage facteur par facteur} & {\small Précision de la conversion cibles $\to$ browniens} \\
\end{longtable}
\vspace{-10pt}
...
\midrule\endhead
\bottomrule\endfoot
{\small Paramètres GSE} & {\small Classeur} & {\small \texttt{Paramètre GSE20}} & {\small -} & {\small 31/12/2025} \\
\rowcolor{nxlightblue}
{\small Matrice de corrélation} & {\small Module Python} & {\small \texttt{Correlation 55}} & {\small -} & {\small 31/12/2025} \\
\end{longtable}
\vspace{-10pt}
...
\midrule\endhead
\bottomrule\endfoot
{\small Génération des chocs corrélés} & {\small MATLAB} & {\small \ph{[module GSE]}} & {\small \ph{[à vérifier]}} \\
\rowcolor{nxlightblue}
{\small Diffusion jointe + sorties} & {\small MATLAB} & {\small \ph{[module GSE]}} & {\small \ph{[à vérifier]}} \\
{\small Restitution (rdt / vol / corrél.)} & {\small Excel} & {\small onglet \texttt{OutPut}} & {\small \ph{[à vérifier]}} \\
\end{longtable}
\vspace{-10pt}
\tabcaption{Localisation des implémentations et statut de vérification}
\subsubsection{Paramètres en production (allocation 2026 vs 2022)}
...
\toprule
\tablehead{Paramètre de simulation} & \tablehead{Valeur} & \tablehead{Commentaire} \\
\midrule\endfirsthead
\toprule
\tablehead{Paramètre de simulation} & \tablehead{Valeur} & \tablehead{Commentaire} \\
\midrule\endhead
\bottomrule\endfoot
{\small Nombre de simulations} & {\small 5\,000} & {\small Onglet GSE20} \\
\rowcolor{nxlightblue}
{\small Pas de temps $dt$} & {\small $1/12$} & {\small Mensuel} \\
{\small Horizon de projection} & {\small 35 ans} & {\small \ph{[ALM $\sim$30 ans à confirmer]}} \\
\rowcolor{nxlightblue}
{\small Schéma CIR (crédit)} & {\small Alfonsi} & {\small } \\
{\small Réserve initiale} & {\small $\approx$ 18,63 Md\euro{}} & {\small } \\
\end{longtable}
\vspace{-10pt}
\tabcaption{Paramètres en production - valeurs extraites du classeur \texttt{Template\_Allocationv2.xlsm}}
\begin{auditbox}
...
\midrule\endhead
\bottomrule\endfoot
{\small Convergence Monte-Carlo} & {\small Stabilité des sorties vs nombre de simulations} & {\small [écart-type MC]} & {\small $\propto 1/\sqrt{N}$} & {\small [OK / KO]} \\
\rowcolor{nxlightblue}
{\small Restitution des moments} & {\small Rendements / volatilités simulés vs cibles} & {\small [écarts]} & {\small $<$ [x] bp} & {\small [OK / KO]} \\
{\small Matrice corrél. simulée vs cible} & {\small Cohérence de la structure de dépendance} & {\small [écarts]} & {\small $<$ [x] pts} & {\small [OK / KO]} \\
\rowcolor{nxlightblue}
{\small Reproductibilité (graine)} & {\small Graine aléatoire fixée} & {\small [OK / KO]} & {\small Tracé} & {\small [OK / KO]} \\
\end{longtable}
\vspace{-10pt}
...
\midrule\endhead
\bottomrule\endfoot
{\small MC-A1} & {\small Erreur Monte-Carlo à 5\,000 trajectoires} & {\small Réduction de variance (antithétiques, quasi-MC) ; intervalle de confiance des sorties} & {\small Précision des indicateurs d'allocation} \\
\rowcolor{nxlightblue}
{\small MC-A2} & {\small Pas mensuel uniforme sur 35 ans} & {\small Contrôle du biais de discrétisation sur les facteurs non exacts (CIR)} & {\small Exactitude des trajectoires} \\
{\small MC-A3} & {\small Horizon de simulation (35 ans) vs horizon ALM ($\sim$30 ans)} & {\small Aligner / documenter les horizons} & {\small Cohérence ALM} \\
\end{longtable}
\vspace{-10pt}
