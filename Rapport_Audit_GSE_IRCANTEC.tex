...
\begin{longtable}{>{\raggedright\arraybackslash}p{1.9cm}>{\raggedright\arraybackslash}p{2.3cm}>{\raggedright\arraybackslash}p{2.7cm}>{\raggedright\arraybackslash}p{2.2cm}>{\raggedright\arraybackslash}p{1.4cm}>{\scriptsize\raggedright\arraybackslash}p{2.6cm}}
\toprule
...
\bottomrule
\endfoot
\rowcolor{nxlightblue} Inflation (CT~/~LT) & Vasicek 2 facteurs & cible COR + Cibles distributionnelles & Schéma exact V2F & R~/~MATLAB & \texttt{GTFRFII2YR} / \texttt{GTFRFII10YR Corp} (BEIR) \\
...
\rowcolor{nxlightblue} Courbe nominale (ZC) & Fisher + corrections de covariance & Param. hérités taux-inflation + primes de risque & Formules fermées V2F & MATLAB & \texttt{GFRN2} / \texttt{GFRN10 Index} \\
Crédit (spread IG) & CIR & {\small Schéma d'Euler puis Initialisation par OLS puis max.\ de vraisemblance ($\chi^2$ décentré)}. & Schéma d'Euler, Alfonsi ou Diop selon pas, pas mensuel (Alfonsi) & MATLAB & \texttt{H06270EU Index} \\
\rowcolor{nxlightblue} Dette privée (spread) & Black-Karasinski & Régression (log-spreads) & Schéma exact log-OU & MATLAB & \texttt{LP01TREU Index} \\
Prime de liquidité & Écart CDS~/~obligataire ; ajust.\ de $\theta$ & Dire d'expert + Solveur d'implicitaion & --- & Excel~/~MATLAB & CDS vs \texttt{LP01TREU} (HY) \\
\rowcolor{nxlightblue} Actions (Euro~/~Monde~/~Émergent) & Hardy RSLN-2 (2 régimes) & Maximum de vraisemblance & BS par régime, pas mensuel & R~/~MATLAB & \texttt{MSDEE15N} / \texttt{MSDEWEUN} / \texttt{MSDEEEMN Index} \\
Action non cotée (PE) & BS (après \emph{unsmoothing}) & \emph{Unsmoothing} AR(1) + formule fermée & Loi normal iid & Excel~/~MATLAB & \texttt{LPX50TR Index} \\
\rowcolor{nxlightblue} Immobilier & BS (après \emph{unsmoothing}) & \emph{Unsmoothing} AR(1) + formule fermée & Loi normal iid & Excel~/~MATLAB & EDHEC IEIF (ieif.fr) \\
Infrastructure & BS & Formule fermée & Loi normal iid & Excel~/~MATLAB & \texttt{NMX30T Index} \\
...
\centering\small
\renewcommand{\arraystretch}{1.4}\setlength{\tabcolsep}{4pt}
\begin{tabularx}{\linewidth}{>{\raggedright\arraybackslash}p{1.7cm} *{8}{>{\centering\arraybackslash}X}}
\toprule
...
\centering\small
\renewcommand{\arraystretch}{1.4}\setlength{\tabcolsep}{5pt}
\begin{tabularx}{\linewidth}{>{\raggedright\arraybackslash}X *{6}{>{\centering\arraybackslash}c}}
\toprule
